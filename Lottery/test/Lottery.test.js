...
describe('Lottery Contract', () => {
    ...
    it('only manager can call pickWinnner', async () => {
        try {
            ...
            assert(false); // if control/execution comes to this line then automatically fail the test no matter what
        } catch(err) {
            assert(err);
        }
    });
    it('send money to the winner and resets the players array', async () => {
        ...
        assert(difference > web3.utils.toWei('1.8', 'ether')) // 1.8 coz we spent some money on gas. it's is close to 2 but just less than 2 due to the gas cost we spent
